...
var formCompileCommand = function(dirName, fileName, language, showWarnings) {
	...
	switch(language) {
		...
		case 'C++':
			warningOption = showWarnings ? '-w ' : '';
			return 'g++ '+warningOption+fileStorePath+dirName+'/'+fileName+'.cpp'+' -o '+'./temp-store/'+dirName+'/'+fileName;
			break;
		case 'Java':
...
var compileFile = function(params, callback) {
	...
	var exec = process.exec(compileCommand, function(error, stdout, stderr) {
		if(error!=null) {
			console.log('In compileFile error');
			response = {
				compileResult 	: "fail",
				error 			: error.toString()
			};
			...
			console.log('In compileFile stderr');
			response = {
				compileResult 	: "fail",
			...
			console.log('In compileFile success');
			response = {
...
compiler.execute = function(data, socket) {
	...
	console.log('pid : ', myProcess.pid);
	compiler.pid = myProcess.pid;
...
compiler.killProcess = function() {
	var killCommand = 'kill '+compiler.pid;
	var exec = process.exec(killCommand, function(error, stdout, stderr) {
		if(error!=null) {
			console.log('Kill process failed. Error : ', error);
		}
		else if(stderr) {
			console.log('Kill process failed. Stderror : ', stderr.toString() );
		}
		else {
